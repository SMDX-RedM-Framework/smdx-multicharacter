...
local function openCharMenu(bool)
    SMDXCore.Functions.TriggerCallback("smdx-multicharacter:server:GetNumberOfCharacters", function(result)
...
RegisterNetEvent('smdx-multicharacter:client:closeNUI', function()
    DeleteEntity(charPed)
    SetNuiFocus(false, false)
    isChossing = false
end)

RegisterNetEvent('smdx-multicharacter:client:chooseChar', function()
...
RegisterNUICallback('cDataPed', function(data) -- Visually seeing the char
    ...
    DeleteEntity(charPed)
    if cData ~= nil then
        SMDXCore.Functions.TriggerCallback('smdx-multicharacter:server:getAppearance', function(appearance)
            ...
            if sex ~= nil then
                CreateThread(function ()
                    ...
                    exports['smdx-appearance']:ApplySkinMultiChar(skinTable, charPed, clothesTable)
                end)
            else
                CreateThread(function()
                    local randommodels = {
...
RegisterNUICallback('disconnectButton', function()
    SetEntityAsMissionEntity(charPed, true, true)
    DeleteEntity(charPed)
    TriggerServerEvent('smdx-multicharacter:server:disconnect')
end)
...
RegisterNUICallback('selectCharacter', function(data)
    ...
    if DataSkin ~= nil then
        DoScreenFadeOut(10)
        TriggerServerEvent('smdx-multicharacter:server:loadUserData', cData)
        openCharMenu(false)
        local model = IsPedMale(charPed) and 'mp_male' or 'mp_female'
        SetEntityAsMissionEntity(charPed, true, true)
        DeleteEntity(charPed)
        Wait(5000)
        TriggerServerEvent("smdx-appearance:loadSkin")
        Wait(500)
        TriggerServerEvent("smdx-clothes:LoadClothes", 1)
        SetModelAsNoLongerNeeded(model)
    else
        DoScreenFadeOut(10)
        TriggerServerEvent('smdx-multicharacter:server:loadUserData', cData, true)
        openCharMenu(false)
        local model = IsPedMale(charPed) and 'mp_male' or 'mp_female'
        SetEntityAsMissionEntity(charPed, true, true)
        DeleteEntity(charPed)
...
RegisterNUICallback('setupCharacters', function() -- Present char info
    SMDXCore.Functions.TriggerCallback("smdx-multicharacter:server:setupCharacters", function(result)
        SendNUIMessage({
...
RegisterNUICallback('createNewCharacter', function(data) -- Creating a char
    ...
    DoScreenFadeOut(150)
    Wait(200)
    TriggerEvent("smdx-multicharacter:client:closeNUI")
    DestroyAllCams(true)
    SetModelAsNoLongerNeeded(charPed)
    DeleteEntity(charPed)
    DoScreenFadeIn(1000)
    FreezeEntityPosition(PlayerPedId(), false)
    TriggerEvent("smdx-appearance:OpenCreator", data)
end)

RegisterNUICallback('removeCharacter', function(data) -- Removing a char
    TriggerServerEvent('smdx-multicharacter:server:deleteCharacter', data.citizenid)
    TriggerEvent('smdx-multicharacter:client:chooseChar')
end)
